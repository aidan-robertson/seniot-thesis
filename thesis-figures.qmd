...
## ⭐CCLU Cover < NOT helpful, only ever 0 or 1 CLPU cover
```{r}
...
success_cover_lm <- lm(cover ~ species * seeding, data = all_seeded)

plot(success_cover_lm)

summary(success_cover_lm) 
...
```{r}
fuel_temp_ss <- fuel_burn_temp %>% filter(tag_depth == "sub-surface")
