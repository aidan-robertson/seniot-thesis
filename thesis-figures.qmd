...
# glm native cover and thatch/non-native
```{r}
## LM native cover and nonnative cover
ggplot(all_seeded) +
         aes(x = nonnative_cover, y = native_cover, color = FALSE) +
...
N3_lm <- lm(native_cover ~ nonnative_cover * thatch_cover, data = all_seeded)
...
NT_lm <- lm(native_cover ~ thatch_cover, data = all_seeded)
summary(NT_lm)


## LM native cover and thatch cover
AS_noburn <- all_seeded %>% filter(! weeding == "Burn")

ggplot(all_seeded) +
         aes(x = thatch_cover, y = native_cover, color = FALSE) +
  geom_smooth(method = "glm", formula = y~x,
              method.args = list(family = gaussian(link = "log"))) +
...
N3_glm <- glm(native_cover ~ thatch_cover, data=all_seeded, family=gaussian(link = "log"))

plot(N3_glm)

summary(N3_glm)

N3T_lm <- lm(native_cover ~ thatch_cover * weeding, data = AS_noburn)
summary(N3T_lm)
```
